...
class scpi (object):
    ...
    def gen_get_settings(self, chan: int, siglab: bool = False) -> List[str]:
        """
    ...
    def acq_get_settings(self, siglab: bool = False, input4: bool = False) -> List[str]:
        """
    ...
    def acq_get_trig_settings(self, siglab: bool = False) -> List[str]:
        """
        Retrieves the standard acquisition settings from Red Pitaya, prints them in console and returns
        them as an array with the following sequence:
    ...
    def _validate_acq_trig_params(
        ...
        """
        # Check for HV gain to determine trigger level limits
        # Use default conservative limit, actual validation done at runtime
        trig_lvl_lim = 20.0  # Maximum possible range (covers both LV and HV)
    ...
    def _validate_acq_split_trig_params(
        ...
        """
        n = 4 if input4 else 2
        # Use maximum possible trigger level (covers both LV and HV gain)
        trig_lvl_lim = 20.0
    ...
    def spi_set(
        ...
        Args:
            ...
            word_len (int, optional): Character size in bits. Defaults to 8.
        """
    ...
    def _validate_spi_params(
        ...
        assert spi_mode in SPIMode, f"{spi_mode.value} is not a defined SPI mode"
        assert cs_mode in SPICSMode, f"{cs_mode.value} is not a defined CS mode"
        assert speed_min_limit <= speed <= speed_max_limit, f"{speed} is out of range [{speed_min_limit},{speed_max_limit}]"
        assert word_len >= bits_min_limit, f"Word length must be greater than {bits_min_limit}. Current word length: {word_len}"
